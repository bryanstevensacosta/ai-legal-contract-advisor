...
from backend.models import HistoricalDecision, ContractType
...
def historical_decision_strategy(draw):
    ...
    contract_types = ["NDA", "MSA", "Service Agreement", "Employment Agreement", "License Agreement"]
    ...
    return HistoricalDecision(
        ...
        clause_modified=draw(st.text(min_size=5, max_size=50, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd', 'P', 'Zs')))),
        original_text=draw(st.text(min_size=20, max_size=200, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd', 'P', 'Zs')))),
        modified_text=draw(st.text(min_size=20, max_size=200, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd', 'P', 'Zs')))),
        rationale=draw(st.text(min_size=20, max_size=200, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd', 'P', 'Zs')))),
        approved_by=draw(st.text(min_size=5, max_size=50, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Zs')))),
        date=datetime.now().isoformat(),
        jurisdiction=draw(st.sampled_from(jurisdictions)),
        confidence=draw(st.floats(min_value=0.0, max_value=1.0, allow_nan=False, allow_infinity=False)),
        tags=draw(st.lists(st.text(min_size=3, max_size=20, alphabet=st.characters(whitelist_categories=('Ll', 'Nd'))), min_size=0, max_size=5)),
        regulatory_basis=draw(st.lists(st.text(min_size=3, max_size=30, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd', 'P'))), min_size=0, max_size=3))
    )
...
class TestProperty18PrecedentPersistence:
    ...
    def test_confidence_score_preserved(self, cloudant_client):
        ...
        for conf in test_confidences:
            ...
            assert retrieved is not None
            assert abs(retrieved.confidence - conf) < 0.0001, f"Confidence {conf} should be preserved"
...
class TestCloudantIntegration:
    ...
    def test_query_golden_clauses(self, cloudant_client):
        ...
        clauses = cloudant_client.query_golden_clauses(
            contract_type="NDA",
            limit=10
        )
    ...
    def test_get_regulatory_mappings(self, cloudant_client):
        ...
        mappings = cloudant_client.get_regulatory_mappings(
            jurisdiction="US",
            limit=10
        )
