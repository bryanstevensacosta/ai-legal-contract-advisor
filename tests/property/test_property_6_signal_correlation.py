...
def internal_signal_strategy(draw):
    ...
    return InternalSignal(
        ...
        type=draw(st.sampled_from(["liability_cap", "indemnification", "confidentiality", "termination"])),
...
def test_signal_correlation_produces_valid_alignments(
    ...
    """
    valid_alignments = {SignalAlignment.MATCH, SignalAlignment.CONFLICT, 
                       SignalAlignment.PARTIAL, SignalAlignment.UNKNOWN}
    
    # Check internal signals
    for signal in internal_signals:
        assert signal.alignment in valid_alignments, \
...
def test_fusion_analysis_contains_all_signal_types(
    ...
    """
    from backend.models import ComplianceGap
...
def test_signal_correlation_structure_is_complete(
    ...
    for signal in internal_signals:
        assert signal.source is not None and len(signal.source) > 0, \
            "Internal signal missing source"
        assert signal.type is not None and len(signal.type) > 0, \
            "Internal signal missing type"
        assert signal.text is not None, \
            "Internal signal missing text"
        assert signal.confidence is not None, \
            "Internal signal missing confidence"
        assert signal.alignment is not None, \
            "Internal signal missing alignment"
    
    # Check external signals have all required fields
    for signal in external_signals:
        assert signal.source is not None and len(signal.source) > 0, \
            "External signal missing source"
        assert signal.regulation is not None and len(signal.regulation) > 0, \
            "External signal missing regulation"
        assert signal.requirement is not None, \
            "External signal missing requirement"
        assert signal.confidence is not None, \
            "External signal missing confidence"
        assert signal.alignment is not None, \
            "External signal missing alignment"
