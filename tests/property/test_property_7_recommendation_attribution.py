...
def test_all_gaps_have_complete_attribution(gaps: List[ComplianceGap]):
    ...
    for gap in gaps:
        ...
        assert gap.clause is not None and len(gap.clause) > 0, \
            "Gap missing clause identifier"
        
        # Check recommendation
        assert gap.recommendation is not None and len(gap.recommendation) > 0, \
            "Gap missing recommendation"
        
        # Check source attribution
        assert gap.regulatory_basis is not None and len(gap.regulatory_basis) > 0, \
...
def test_all_actions_have_complete_attribution(actions: List[RecommendedAction]):
    ...
    for action in actions:
        # Check clause identifier
        assert action.clause is not None and len(action.clause) > 0, \
            "Action missing clause identifier"
        
        # Check action type
        assert action.action is not None and action.action in valid_actions, \
            "Action missing or invalid action type"
        
        # Check rationale (source attribution)
        assert action.rationale is not None and len(action.rationale) > 0, \
