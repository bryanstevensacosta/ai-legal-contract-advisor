...
class CloudantStatusChecker:
    ...
    def check_all_databases(self):
        ...
        databases = {
            ...
            'historical_decisions': os.getenv('CLOUDANT_DB_HISTORICAL_DECISIONS', 'historical_decisions'),
            'regulatory_mappings': os.getenv('CLOUDANT_DB_REGULATORY_MAPPINGS', 'regulatory_mappings')
        }
            ...
            if db_status['exists']:
                ...
                if idx_status.get('exists'):
                    if idx_status['count'] > 0:
                        ...
                        print(f"     ⚠️  NO custom indexes found (only default _all_docs)")
                else:
                    print(f"     ❌ Could not check indexes: {idx_status.get('error', 'Unknown error')}")
            else:
        ...
        all_have_indexes = all(
            ...
            for r in results.values() if r['database']['exists']
        )
...
def main():
    ...
    try:
        ...
        all_ready = all(
            r['database']['exists'] and 
            r['indexes'].get('exists') and 
            r['indexes'].get('count', 0) > 0
