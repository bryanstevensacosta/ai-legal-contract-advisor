...
class MemoryQueryRequest(BaseModel):
    ...
    limit: int = Field(default=10, ge=1, le=50, description="Maximum number of precedents to return")
...
async def query_precedents(request: MemoryQueryRequest):
    ...
    try:
        ...
        for prec in precedents_data:
            try:
                ...
                historical_signal = HistoricalSignal(
                    ...
                    contract_type=ContractType(prec.get("contract_type", request.contract_type.value)),
        ...
        avg_confidence = (
            sum(p.confidence for p in precedents) / len(precedents)
