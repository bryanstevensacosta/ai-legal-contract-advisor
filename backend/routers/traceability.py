...
class TraceRequest(BaseModel):
    ...
    precedents: List[HistoricalSignal] = Field(default_factory=list, description="Historical precedents")
...
async def generate_trace(request: TraceRequest):
    ...
    try:
        ...
        # Summary
        trace_parts.append(_generate_summary(request.fusion_analysis, request.routing_decision, recommendations))
...
def _generate_signal_analysis(fusion_analysis: FusionAnalysis, precedents: List[HistoricalSignal]) -> str:
    ...
    if precedents:
        ...
        avg_confidence = sum(p.confidence for p in precedents) / len(precedents) if precedents else 0
        lines.append(f"  Average Confidence: {avg_confidence:.2f}")
        lines.append(f"  Status: SUPPORTS ANALYSIS")
...
def _generate_risk_assessment(routing_decision: RoutingDecision) -> str:
    ...
    lines.append(f"Human Review Required: {'YES' if routing_decision.human_review_required else 'NO'}")
...
def _generate_recommendations(fusion_analysis: FusionAnalysis) -> List[RecommendedAction]:
...
def _format_recommendations(recommendations: List[RecommendedAction]) -> str:
    ...
    for i, rec in enumerate(recommendations, 1):
        ...
        if rec.recommended_text:
            lines.append(f"\n**Recommended Text:**")
            lines.append(rec.recommended_text[:200] + "..." if len(rec.recommended_text) > 200 else rec.recommended_text)
