...
class ContractAnalysisRequest(BaseModel):
    ...
    clauses: List[ContractClause] = Field(default_factory=list, description="Extracted contract clauses")
...
Golden Clause ({golden.get('type', 'unknown')}):
{golden.get('text', '')}
...
Format as JSON: {{"alignment": "...", "confidence": 0.0, "explanation": "..."}}"""
            ...
            internal_signals.append(InternalSignal(
                source=f"Golden Clause #{golden.get('clause_id', 'unknown')}",
                type=golden.get('type', 'unknown'),
                text=golden.get('text', '')[:200],  # Truncate for response size
                confidence=confidence,
                alignment=alignment
            ))
            
        except Exception as e:
            print(f"Warning: Failed to analyze Golden Clause {golden.get('clause_id')}: {e}")
            continue
...
def _calculate_overall_confidence(
    ...
    all_confidences = (
        [s.confidence for s in internal_signals] +
